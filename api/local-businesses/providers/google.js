...
const FIELD_MASK = ['places.id','places.displayName','places.primaryType','places.types','places.rating','places.userRatingCount','places.formattedAddress','places.nationalPhoneNumber','places.internationalPhoneNumber','places.websiteUri','places.businessStatus','places.googleMapsUri','places.location','nextPageToken'].join(',');
...
async function searchGoogle(criteria, options = {}) {
  ...
  return { provider:'google', businesses:(data.places || []).map(normaliseGooglePlace), nextPageToken:data.nextPageToken || null };
}

function normaliseGooglePlace(place = {}) { const website=text(place.websiteUri); return { id:text(place.id),name:text(place.displayName?.text),category:humanise(place.primaryType || place.types?.[0] || ''),types:Array.isArray(place.types)?place.types.filter((v)=>typeof v==='string'):[],rating:Number.isFinite(place.rating)?place.rating:null,reviewCount:Number.isFinite(place.userRatingCount)?place.userRatingCount:null,address:text(place.formattedAddress),phone:text(place.nationalPhoneNumber),internationalPhone:text(place.internationalPhoneNumber),website,websiteStatus:website?'not_checked':'no_website',businessStatus:text(place.businessStatus),source:'google',sourceUrl:text(place.googleMapsUri),latitude:numberOrNull(place.location?.latitude),longitude:numberOrNull(place.location?.longitude),openingHours:'',tags:{}}; }
