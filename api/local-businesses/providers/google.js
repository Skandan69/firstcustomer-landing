...
async function searchGoogle(criteria, options = {}) {
  ...
  const body = { textQuery: `${searchTerm} in ${criteria.location}`, languageCode: 'en', maxResultCount: 20 };
  ...
  return { provider:'google', businesses:(data.places || []).map(place=>normaliseGooglePlace(place,selected)).filter(Boolean), nextPageToken:data.nextPageToken || null,categoryMatch:{recognized:selected.recognized,id:selected.id||'',label:selected.label||'',bestEffort:false} };
}

function normaliseGooglePlace(place = {},selected=resolveCategory('')) { const website=text(place.websiteUri);const component=(type)=>text((place.addressComponents||[]).find((item)=>item.types?.includes(type))?.longText); return normalizeBusiness({provider:'google',providerId:text(place.id),name:text(place.displayName?.text),category:selected.recognized?selected.label:humanise(place.primaryType || place.types?.[0] || ''),matchedCategory:selected.input?(selected.id||selected.input):'',matchReason:selected.input?`Google Places targeted search for ${selected.googleTerm||selected.input}`:'Google Places business result',matchConfidence:selected.input?0.9:0.7,types:place.types,address:text(place.formattedAddress),city:component('locality')||component('administrative_area_level_2'),state:component('administrative_area_level_1'),country:component('country'),latitude:numberOrNull(place.location?.latitude),longitude:numberOrNull(place.location?.longitude),phone:text(place.internationalPhoneNumber||place.nationalPhoneNumber),email:'',website,rating:Number.isFinite(place.rating)?place.rating:null,reviewCount:Number.isFinite(place.userRatingCount)?place.userRatingCount:null,businessStatus:text(place.businessStatus),sourceUrl:text(place.googleMapsUri),openingHours:''}); }
