...
async function handleLoadMore() {
  ...
  try { const result = await searchLocalBusinesses({ ...state.criteria, provider:state.provider==='google'?'google':'open_data', pageToken: state.nextPageToken }); state.businesses = dedupe([...state.businesses, ...result.businesses]); state.nextPageToken = result.nextPageToken; state.provider=result.provider; refreshDerivedUi(); renderResults(); }
